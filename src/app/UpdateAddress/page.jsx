...
export default function UpdateAddress({updateName,setUpdateForm,setAddresssData,AddresssData}){
...
const FormSubmitHandler = (values) => {
  ...
  const updatedData = [...AddresssData];
  const index = updatedData.findIndex((item) => item.name === updateName.name);
  if (index !== -1) {
    updatedData[index] = { ...updateName, ...values };
  } else {
    updatedData.push({ ...updateName, ...values });
  }
  setAddresssData(updatedData);
  localStorage.setItem('AddresssData', JSON.stringify(updatedData));
  setUpdateForm(false)
};
return (
    <div className="py-2">
      <Formik
        initialValues={{
          ...
          radiobtn: updateName?.radiobtn,
          addresslineone: updateName?.addresslineone,
          addresslinetwo: updateName?.addresslinetwo,
          addresslinethree:updateName?.addresslinethree,
          landmark:updateName?.landmark,
          country:updateName?.country,
          state:updateName?.state,
          city: updateName?.city,
          pincode:updateName?.pincode,
          mobile:updateName?.mobile,
        }}
